 ...
 * Based on Mercury's Testing/common/na_test.h with MPI/CXI removed.
 */
...
struct na_test_info {
    ...
    int port;                /* Port */
    bool listen;             /* Listen */
    bool mpi_static;         /* MPI static comm (unused) */
...
/**
 * Call MPI_Barrier if available (no-op in standalone mode)
 */
void
NA_Test_barrier(const struct na_test_info *na_test_info);

/**
 * Call MPI_Bcast if available (no-op in standalone mode)
 */
void
NA_Test_bcast(
