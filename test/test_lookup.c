/**
 * NA lookup test client - adapted for ZMQ NA plugin standalone testing.
 * Based on Mercury's Testing/unit/na/test_lookup.c.
 *
 * Reads server addresses from config file, sends expected messages
 * in a loop to all targets, then sends a finalize (DONE) message.
 */
    ...
    size_t target_name_count, int NA_UNUSED thread_id)
{
    ...
    for (j = 0; j < loop; j++) {
        for (i = 0; i < (int) target_name_count; i++) {
            ...
            /* Post one-way msg send (unexpected) */
