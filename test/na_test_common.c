/**
 * NA test common utilities - adapted for ZMQ NA plugin standalone testing.
 * Based on Mercury's Testing/unit/na/na_test_common.c.
 */
    ...
    struct na_test_common_class_info *info, na_class_t *na_class, bool listen)
{
    ...
    if (listen) {
        ...
        NA_TEST_CHECK_NA_ERROR(error, ret,
            "NA_Msg_init_expected() failed (%s)", NA_Error_to_string(ret));
    }
