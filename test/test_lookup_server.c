/**
 * NA lookup test server - adapted for ZMQ NA plugin standalone testing.
 * Based on Mercury's Testing/unit/na/test_lookup_server.c.
 *
 * Listens for unexpected messages from clients, processes them until
 * a DONE tag is received, then exits.
 */
    ...
    na_addr_t *source, na_tag_t tag)
{
    ...
    switch (tag) {
        case NA_TEST_COMMON_TAG_DONE:
            printf("# Server received DONE tag\n");
            recv_info->done = true;
            break;
        case NA_TEST_COMMON_TAG_CONTINUE:
            break;
        default:
            fprintf(stderr, "# Server received unknown tag: %u\n", tag);
