/**
 * NA Test infrastructure - adapted for ZMQ NA plugin standalone testing.
 * Based on Mercury's Testing/common/na_test.c with MPI/CXI removed.
 */
...
extern int na_test_opt_ind_g;
extern const char *na_test_opt_arg_g;
...
na_test_parse_options(int argc, char *argv[], struct na_test_info *na_test_info)
{
    ...
    /* Allow running without arguments (use defaults) */
    ...
    /* Default to zmq protocol if not specified */
    if (!na_test_info->protocol)
        na_test_info->protocol = strdup("zmq");
...
na_test_set_config(const char *hostfile, const char *addr_name, bool append)
{
    ...
    NA_TEST_CHECK_ERROR(config == NULL, error, ret, NA_NOENTRY,
        "Could not open config file from: %s", config_file);
...
NA_Test_init(int argc, char *argv[], struct na_test_info *na_test_info)
{
    ...
    /* Initialize MPI stub (no-op for standalone) */
    ...
    for (i = 0; i < na_test_info->max_classes; i++) {
        info_string = na_test_gen_config(na_test_info, (unsigned int) i);
    ...
    na_test_info->na_class = na_test_info->na_classes[0];
    ...
    /* Get config from file if self option is not passed */
        ...
        NA_TEST_CHECK_ERROR(na_test_info->target_names == NULL, error, ret,
            NA_NOMEM, "Could not allocate target name array");

        {
            size_t c = na_test_info->max_targets;

            ret = na_test_get_config(
                na_test_info->hostfile, na_test_info->target_names, &c);
            NA_TEST_CHECK_NA_ERROR(error, ret,
                "na_test_get_config() failed (%s)", NA_Error_to_string(ret));
        }
...
/*---------------------------------------------------------------------------*/
void
NA_Test_barrier(const struct na_test_info NA_UNUSED *na_test_info)
{
...
/*---------------------------------------------------------------------------*/
void
NA_Test_bcast(void NA_UNUSED *buf, size_t NA_UNUSED size, int NA_UNUSED root,
    const struct na_test_info NA_UNUSED *na_test_info)
{
    /* No-op for standalone */
}
